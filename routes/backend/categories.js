...
let utilHelper = require(__path_helper + "utilsCategory"); 
let validateHelper = require(__path_validates + "categories"); 
...
const folderView = "pages/categories/"; 
...
router.get('/form(/:id)?', async (req, res, next) =>  {
  ...
  if(currentId === undefined || currentId === ""){
    res.render(`${folderView}add`, { title: 'Categories Management-Add', item: itemDefault, errors});
  } else {
    await  CategoriesModel.getItem(currentId).then((itemEdit)=>{
      res.render(`${folderView}add`, { title: 'Categories Management - Edit', item: itemEdit, errors });
     }) 
  }
});
...
let errors =  validationResult(req); 
 ...
 if(id === "" || id === undefined){
   if(errors.length > 0){
    res.render(`${folderView}add`, { title: 'Categories Management - Add', item: item,errors });
     return 
   } else {
    await CategoriesModel.saveItem(id, item, {task: "add"}).then(()=>{  
   ...
   if(errors.length > 0) {
    res.render(`${folderView}add`, { title: 'Categories Management - Edit', item: item,errors });
    return 
   } else {
    await  CategoriesModel.saveItem(id, item,{task: "edit"}).then( (result)=>{ 
...
router.get('(/:status)?', async (req, res, next)  => {
  ...
  await CategoriesModel
         .listItems(objectFilter, params)          
          .then((items)=>{
          res.render(`${folderView}list`, { title: 'Categories Management List', 
          items: items,
          statusFilter, 
          params  });       
});
...
//change multi status 
  ...
  let idArray = req.body.cid;
    await CategoriesModel.changeStatus(idArray, currentStatus, {task: "update-multi"})
    .then((result)=>{
    ...
    router.get('/change-status/:status/:id/', async (req, res, next)  => {
      let currentStatus = await utilHelper.getParams(req.params, "status", "all");  
      let currentID = await utilHelper.getParams(req.params, "id", ""); 
      await CategoriesModel.changeStatus(currentID, currentStatus, {task: "update-one"})
      .then( (result)=>{
...
//change group acp
 router.get('/change-group/:group_acp/:id/', async (req, res, next)  => {
  ...
  let currentID = await utilHelper.getParams(req.params, "id", ""); 
  await CategoriesModel.changeGroupACP(currentGroup, currentID)
  .then( (result)=>{
 ...
 router.get('/delete/:id/', async (req, res, next)  => {  
  ...
  await CategoriesModel.deleteItem(currentID, {task: "delete-one"})
  .then((result)=> {
...
router.post('/delete/', async (req, res, next)  => {
  let idArray = req.body.cid
      await CategoriesModel.deleteItem(idArray, {task: "delete-many"})
      .then((result)=>{ 
...
router.post('/change-ordering/', async  (req, res, next)  => {
  ...
  await  CategoriesModel.changeOrdering(idArray, ordering)
    .then((result)=>{
